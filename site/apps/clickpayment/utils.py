...
from apps.order.models import CustomOrder, OrderItem, Product
...
class PyClickMerchantAPIView(APIView):
    ...
    def complete(cls, click_trans_id: str,
        ...
        if error and int(error) < 0:
            ...
            custom_order = CustomOrder.objects.get(id=order.order_id)
            custom_order.status = CustomOrder.OrderStatus.REJECTED
            custom_order.save()
            text = f"Buyurtma №:{order.order_id} bekor qilindi"
            send_bot_message(message=text)
        if result['error'] == SUCCESS:
            order.change_status(ClickTransaction.CONFIRMED)
            order.click_paydoc_id = click_paydoc_id
            order.save()
            custom_order = CustomOrder.objects.get(id=order.order_id)
            custom_order.is_paid =True
            custom_order.save()
            text = f"Buyurtma №:{order.order_id} to'landi"
            send_bot_message(message=text)
            cls.VALIDATE_CLASS().successfully_payment(order)
            fiscal_items = []
            custom_order_items = OrderItem.objects.filter(order=custom_order).select_related('product')
            for order_items in custom_order_items:
                item = dict()
                item['Name'] = f"{order_items.product.title} ({order_items.product.measure})"  
                item['SPIC'] = f"{order_items.product.mxik}"  
                item['Units'] = order_items.product.units_code  
                item['Price'] = order_items.quantity * order_items.price / order_items.org_count_in_measure
                item['Amount'] = order_items.quantity
                item['VAT'] = 0
                item['VATPercent'] = 0
                item['CommissionInfo'] = {"TIN": f"{os.getenv('INN')}"}
                fiscal_items.append(item)
            result['fiscal_items'] = fiscal_items
        result['click_trans_id'] = click_trans_id
        result['merchant_trans_id'] = merchant_trans_id
