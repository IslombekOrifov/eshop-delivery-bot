        ...
        if isinstance(transaction, ClickTransaction):
            ...
            if check_invoice.status_code == 200:
                ...
                if _json['status'] > 0:
                    ...
                    order = CustomOrder.objects.get(transaction_id=transaction.id)
                    if order.exists():
                        order.status = CustomOrder.OrderStatus.PROCESS
                        order.is_paid = True
                elif _json['status'] == -99:
                    transaction.change_status(ClickTransaction.CANCELED)
                    order = CustomOrder.objects.get(transaction_id=transaction.id)
                    if order.exists():
                        order.status = CustomOrder.OrderStatus.REJECTED
                elif _json['status'] < 0:
                    transaction.change_status(ClickTransaction.ERROR)
                    order = CustomOrder.objects.get(transaction_id=transaction.id)
                    if order.exists():
                        order.status = CustomOrder.OrderStatus.ERROR
                transaction.message = json.dumps(_json)
                transaction.save()
                return _json
